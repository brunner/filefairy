...
EVENT_MAP = {
    ...
    (r'^\d-\d: Fielders Choice at {base}, {scoring} \(G'
     r'roundball, {zone}\)$'),
    ...
    r'^Pickoff Throw by Catcher to {base} - Out!$',
    Event.FIELDER_THROWING:
    r'^Throwing error, {scoring}$',
    Event.PITCHER_PICK_ERROR:
    r'^Pickoff Throw to {base} - Error! {scoring}$',
    Event.PITCHER_PICK_OUT:
    r'^Pickoff Throw to {base} - Out! {scoring}$',
    Event.PITCHER_BALK:
    r'^Balk!$',
    ...
    r'^\d-\d: Bunted foul$',
    Event.PITCHER_STRIKE_FOUL_ERROR:
    ...
    Event.PITCHER_STRIKE_SWING_WILD: (r'^\d-\d: Strikes out swinging wild pitc'
                                      r'h, reaches first!$'),
    Event.RUNNER_STEAL:
    r'^{player} steals {base} base(?: \(no throw\))?$',
