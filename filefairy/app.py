...
class App(object):
    ...
    def listen(self):
        def on_message_(ws, message):
            ...
            if all(k in obj for k in ['type', 'channel', 'text'
                ...
                if obj['channel'] == self.get_statsplus_id():
                    if 'bot_id' in obj:
                        bot_id = obj['bot_id']
                        if bot_id == self.get_realtime_score_id(
                        ) or bot_id == self.get_injury_bot_id():
        ...
        if obj['ok'] and 'url' in obj:
            self.ws = websocket.WebSocketApp(
                obj['url'], on_message=on_message_)
    ...
    def handle_testing(self, text):
        ...
        if text == 'Run app status report.':
            ...
            files_upload(self.format_standings_al(), 'AL.txt',
                         self.get_testing_name())
            files_upload(self.format_standings_nl(), 'NL.txt',
                         self.get_testing_name())
            deb = ', '.join([
                '{}: {}'.format(k.title(), self.settings[k])
    ...
    def process_final_scores(self):
        ...
        for date in dates:
            ...
            chat_post_message(self.get_live_sim_discussion_name(),
                              date + '\n' + scores)
            ...
            for t, u in zip([35, 44, 48], [36, 45, 49]):
                ...
                ws = re.findall(r'\|' + re.escape(city) + '\s\d+,\s([^\d]+)\d',
                                scores)
    ...
    """
        ...
        for v in self.get_ordered(s, group, 's'):
            ...
            gb = '{:0.1f}'.format(abs(dgb)) if dgb < 0 else '+{:0.1f}'.format(
                dgb) if dgb > 0 else '-'
            mn = 'X' if s[v].mn[k] == 0 else '' if not s[v].mn[k] else s[v].mn[
                k]
            lines.append(
                row.format(city=city, div=div, w=w, l=l, gb=gb, mn=mn))
    ...
    """
        ...
        keast, kcent, kwest, kwc = [
            k + ' ' + ey for ey in ['East', 'Central', 'West', 'Wild Card']
        ]
        ...
        groups = zip([keast, kcent, kwest, kwc], [east, cent, west, wc],
                     ['d', 'd', 'd', 'w'])
        ret = [
            self.get_standings_output(s, kgroup, group, k)
            for (kgroup, group, k) in groups
        ]
        return '\n\n'.join(ret)

    def format_standings_al(self):
        aleast, alcent, alwest = [33, 34, 48, 57,
                                  59], [35, 38, 40, 43,
                                        47], [42, 44, 50, 54, 58]
        return self.format_standings_internal(aleast, alcent, alwest, 'AL')

    def format_standings_nl(self):
        nleast, nlcent, nlwest = [32, 41, 49, 51,
                                  60], [36, 37, 46, 52,
                                        56], [31, 39, 45, 53, 55]
