...
class DatetimeTest(unittest.TestCase):
    def test_datetime_datetime_minimal(self):
        actual = datetime_datetime(2018, 3, 13)
        expected = _est.localize(datetime.datetime(2018, 3, 13))
        self.assertEqual(actual, expected)

    def test_datetime_datetime_full(self):
        actual = datetime_datetime(2018, 3, 13, 22, 43, 13, 337756)
    ...
    def test_datetime_now(self, mock_datetime):
        ...
        actual = datetime_now()
        expected = _est.localize(date)
        self.assertEqual(actual, expected)
        mock_datetime.datetime.now.assert_called_once_with()

    def test_decode(self):
        actual = decode_datetime('2018-03-13T22:43:13.337756-04:00')
        expected = _est.localize(
            datetime.datetime(2018, 3, 13, 22, 43, 13, 337756))
        self.assertEqual(actual, expected)
    ...
    def test_encode(self):
        actual = encode_datetime(
            _est.localize(datetime.datetime(2018, 3, 13, 22, 43, 13, 337756)))
        expected = '2018-03-13T22:43:13.337756-04:00'
        self.assertEqual(actual, expected)
