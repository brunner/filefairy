...
sys.path.extend((_path, re.sub(r'/tasks/scoreboard', '', _path)))

from data.notify.notify import Notify  # noqa
from data.response.response import Response  # noqa
from data.shadow.shadow import Shadow  # noqa
from data.thread_.thread_ import Thread  # noqa
from tasks.scoreboard.scoreboard import Scoreboard  # noqa
...
EXTRACT_DIR = re.sub(r'/tasks/scoreboard', '/resource/extract', _path)
GAMES_DIR = re.sub(r'/tasks/scoreboard', '/resource/games', _path)
RESOURCE_DIR = re.sub(r'/tasks/scoreboard', '/resource', _path)
...
_injuries_clarified = [
    'SP <{0}{1}37102.html|Jairo Labou> was injured while pitching (T54 @ T' +
    '34)',
...
_injuries_text = 'SP <{0}{1}37102.html|Jairo Labou> was injured while pi' + \
...
class ScoreboardTest(Test):
    def setUp(self):
        ...
        patch_chat = mock.patch.object(Scoreboard, '_chat')
        self.addCleanup(patch_chat.stop)
        self.mock_chat = patch_chat.start()

        patch_log = mock.patch('tasks.scoreboard.scoreboard.logger_.log')
    ...
    def create_scoreboard(self, data):
        self.init_mocks(data)
        scoreboard = Scoreboard(date=_now, e=_env)
        scoreboard.shadow['download.end'] = _now_encoded

        self.mock_open.assert_called_once_with(Scoreboard._data(), 'r')
    ...
    def test_notify__with_finish(self):
        ...
        self.mock_open.assert_called_with(Scoreboard._data(), 'w')
    ...
    def test_notify__with_year(self):
        scoreboard = self.create_scoreboard(_data())
        response = scoreboard._notify_internal(notify=Notify.DOWNLOAD_YEAR)
        self.assertEqual(response, Response())
    ...
    def test_notify__with_other(self):
        scoreboard = self.create_scoreboard(_data())
        response = scoreboard._notify_internal(notify=Notify.OTHER)
        self.assertEqual(response, Response())
    ...
    @mock.patch.object(Scoreboard, '_handle_table')
    @mock.patch.object(Scoreboard, '_render')
    @mock.patch.object(Scoreboard, '_handle_key')
    @mock.patch.object(Scoreboard, '_clear')
    ...
    def test_on_message__with_scores_and_postseason(
        ...
        scoreboard = self.create_scoreboard(
            _data(offseason=True, postseason=True))
        response = scoreboard._on_message_internal(obj=obj)
        self.assertEqual(
            response,
    ...
    def test_on_message__with_table_and_postseason(
        ...
        scoreboard = self.create_scoreboard(_data(offseason=True))
        response = scoreboard._on_message_internal(obj=obj)
        self.assertEqual(
            response,
            Response(notify=[Notify.BASE], shadow=scoreboard._shadow_data()))
    ...
    def test_run__with_extractd(self, mock_render):
        scoreboard = self.create_scoreboard(_data())
        response = scoreboard._run_internal(date=_now)
        self.assertEqual(response, Response())
    ...
    def test_run__with_unchecked(self, mock_render):
        scoreboard = self.create_scoreboard(_data(unchecked=[_then_encoded]))
        response = scoreboard._run_internal(date=_now)
    ...
    @mock.patch('tasks.scoreboard.scoreboard.check_output')
    def test_clear(self, mock_check):
        ...
        scoreboard = self.create_scoreboard(read)
        scoreboard._clear()
        ...
        self.mock_chat.assert_not_called()
        self.mock_log.assert_not_called()
        self.assertEqual(scoreboard.data['highlights'], {})
        self.assertEqual(scoreboard.data['injuries'], {})
        self.assertEqual(scoreboard.data['scores'], {})
    ...
    def test_handle_key__delay(self):
        ...
        self.assertEqual(scoreboard.data['unchecked'], [])
    ...
    def test_handle_key__scores(self):
        ...
        self.assertEqual(
            scoreboard.data['unchecked'], [[_then_encoded, id_] for id_ in [
    ...
    def test_handle_table(self):
        scoreboard = self.create_scoreboard(_data())
        scoreboard._handle_table(_then_encoded, _table_text)

        self.mock_open.assert_not_called()
        self.mock_handle.write.assert_not_called()
        self.mock_chat.assert_not_called()
        self.mock_log.assert_not_called()
        self.assertEqual(scoreboard.data['table'],
                         {_then_encoded: _table_encoded_new})
        self.assertEqual(scoreboard.data['unchecked'], [])

    @mock.patch.object(Scoreboard, '_table')
    @mock.patch('tasks.scoreboard.scoreboard.standings_table')
    @mock.patch.object(Scoreboard, '_live_postseason')
    @mock.patch.object(Scoreboard, '_forecast')
    def test_home__with_postseason(self, mock_forecast, mock_live,
        ...
        scoreboard = self.create_scoreboard(read)
        actual = scoreboard._home(date=_now)
        breadcrumbs = [{
            'href': '/',
            'name': 'Fairylab'
        }, {
    ...
    @mock.patch.object(Scoreboard, '_table')
    @mock.patch('tasks.scoreboard.scoreboard.standings_table')
    @mock.patch.object(Scoreboard, '_live_regular')
    @mock.patch.object(Scoreboard, '_forecast')
    ...
    @mock.patch('tasks.scoreboard.scoreboard.teamids')
    @mock.patch.object(Scoreboard, '_record')
    def test_forecast__with_empty(self, mock_record, mock_teamids):
        ...
        scoreboard = self.create_scoreboard(read)
        scoreboard.shadow['recap.standings'] = {}
        actual = scoreboard._forecast()
    ...
    def test_forecast__with_valid_input(self, mock_record, mock_teamids):
        ...
        scoreboard = self.create_scoreboard(read)
        scoreboard.shadow['recap.standings'] = _standings_old
        actual = scoreboard._forecast()
    ...
    @mock.patch.object(Scoreboard, '_live_postseason_body')
    def test_live_postseason(self, mock_body):
        ...
        scoreboard = self.create_scoreboard(read)
        actual = scoreboard._live_postseason()
        ps_header = table(
    ...
    @mock.patch.object(Scoreboard, '_live_postseason_series')
    @mock.patch.object(Scoreboard, '_record')
    @mock.patch('tasks.scoreboard.scoreboard.logo_absolute')
    def test_live_postseason_body(self, mock_logo, mock_record, mock_series):
        ...
        scoreboard = self.create_scoreboard(_data())
        actual = scoreboard._live_postseason_body()
        expected = [
            [
                cell(content='logo'),
                cell(content='1'),
    ...
    def test_live_postseason_series(self):
        read = _data(
            ...
            scores={_then_encoded: _scores_postseason_encoded})
        scoreboard = self.create_scoreboard(read)
        actual = scoreboard._live_postseason_series()
    ...
    @mock.patch.object(Scoreboard, '_live_regular_body')
    @mock.patch('tasks.scoreboard.scoreboard.divisions')
    def test_live_regular(self, mock_divisions, mock_body):
        ...
        scoreboard = self.create_scoreboard(_data())
        actual = scoreboard._live_regular()
        al_header = table(
            clazz='table-fixed border border-bottom-0 mt-3',
            hcols=[col(clazz='text-center')],
    ...
    @mock.patch.object(Scoreboard, '_record')
    @mock.patch('tasks.scoreboard.scoreboard.logo_inline')
    def test_live_regular_body(self, mock_logo, mock_record):
        ...
        al = [('AL East', ['33', '34', '48']),
              ('AL Central', ['35', '38', '40']),
              ('AL West', ['42', '44', '50'])]
        actual = scoreboard._live_regular_body(al)
    ...
    def test_record__with_encoded(self):
        ...
        scoreboard = self.create_scoreboard(read)
        self.assertEqual(scoreboard._record('31'), '1-0')
        self.assertEqual(scoreboard._record('32'), '1-0')
        self.assertEqual(scoreboard._record('33'), '0-1')
        self.assertEqual(scoreboard._record('34'), '0-1')
        self.assertEqual(scoreboard._record('35'), '0-1')
        self.assertEqual(scoreboard._record('36'), '0-1')
        self.assertEqual(scoreboard._record('37'), '1-0')
        self.assertEqual(scoreboard._record('38'), '0-1')
        self.assertEqual(scoreboard._record('39'), '0-1')
        self.assertEqual(scoreboard._record('40'), '1-0')
        self.assertEqual(scoreboard._record('41'), '1-0')
        self.assertEqual(scoreboard._record('42'), '1-0')
        self.assertEqual(scoreboard._record('43'), '1-0')
        self.assertEqual(scoreboard._record('44'), '0-1')
        self.assertEqual(scoreboard._record('45'), '0-1')
        self.assertEqual(scoreboard._record('46'), '0-1')
        self.assertEqual(scoreboard._record('47'), '0-1')
        self.assertEqual(scoreboard._record('48'), '1-0')
        self.assertEqual(scoreboard._record('49'), '1-0')
        self.assertEqual(scoreboard._record('50'), '0-1')
        self.assertEqual(scoreboard._record('51'), '1-0')
        self.assertEqual(scoreboard._record('52'), '0-1')
        self.assertEqual(scoreboard._record('53'), '1-0')
        self.assertEqual(scoreboard._record('54'), '0-1')
        self.assertEqual(scoreboard._record('55'), '0-1')
        self.assertEqual(scoreboard._record('56'), '1-0')
        self.assertEqual(scoreboard._record('57'), '1-0')
        self.assertEqual(scoreboard._record('58'), '1-0')
        self.assertEqual(scoreboard._record('59'), '1-0')
        self.assertEqual(scoreboard._record('60'), '0-1')
    ...
    @mock.patch.object(Scoreboard, '_render')
    @mock.patch('tasks.scoreboard.scoreboard.parse_player')
    @mock.patch('tasks.scoreboard.scoreboard.open')
    @mock.patch('tasks.scoreboard.scoreboard.parse_game_data')
    def test_extract_all__with_valid_box_finished_false(
        ...
        read = _data(
            ...
            unchecked=unchecked)
        scoreboard = self.create_scoreboard(read)
        response = scoreboard._extract_all(unchecked, date=_then)
        self.assertEqual(response, Response())
        ...
        mock_data.assert_has_calls([
            mock.call(_then, id_.format(_html, _game_box, '.html'),
        ...
        self.mock_open.assert_called_once_with(Scoreboard._data(), 'w')
        self.mock_handle.write.assert_called_once_with(dumps(write) + '\n')
        self.mock_chat.assert_not_called()
        self.mock_log.assert_not_called()
    ...
    def test_extract_all__with_valid_box_finished_true(
        ...
        mock_data.assert_has_calls([
            mock.call(_then, id_.format(EXTRACT_DIR, _game_box, '.html'),
                      id_.format(EXTRACT_DIR, _game_log, '.txt'))
            for id_ in ids
        ])
        link = '{0}{1}29663.html'.format(_html, _player)
        mock_player.assert_called_once_with(link)
    ...
    def test_table__highlights(self):
        ...
        actual = scoreboard._table('highlights', _then_encoded, _player)
        body = [[
            cell(
                content=_player_sub(
    ...
    def test_table__injuries(self):
        ...
        actual = scoreboard._table('injuries', _then_encoded, _player)
        body = [
            [
                cell(
                    content=_player_sub('SP <a href="{0}{1}37102.html">Jairo L'
                                        'abou</a> was injured while pitching ('
                                        'Seattle Mariners @ Boston Red Sox)'))
            ],
            [
                cell(
                    content=_player_sub('SS <a href="{0}{1}29923.html">Jeremy '
                                        'Houston</a> was injured while running'
                                        ' the bases (Seattle Mariners @ Boston'
                                        ' Red Sox)'))
            ],
            [
                cell(
                    content=_player_sub('CF <a href="{0}{1}1473.html">Jeren Ke'
                                        'ndall</a> was injured while running t'
                                        'he bases (Arizona Diamondbacks @ Los '
                                        'Angeles)'))
            ],
            [
                cell(
                    content=_player_sub('SP <a href="{0}{1}29663.html">Dakota '
                                        'Donovan</a> was injured while pitchin'
                                        'g (New York @ Los Angeles)'))
            ],
        ]
        expected = table(
            head=[cell(content='Sunday, October 9th, 2022')], body=body)
    ...
    def test_table__scores(self):
        ...
        actual = scoreboard._table('scores', _then_encoded, _game_box)
        body = [
            [
                cell(
                    content=_game_box_sub('<a href="{0}{1}2998.html">Arizona D'
                                          'iamondbacks 4, Los Angeles 2</a>'))
            ],
            [
                cell(
                    content=_game_box_sub('<a href="{0}{1}3003.html">Atlanta B'
                                          'raves 2, Baltimore Orioles 1</a>'))
            ],
            [
                cell(
                    content=_game_box_sub('<a href="{0}{1}2996.html">Cincinnat'
                                          'i Reds 7, Milwaukee Brewers 2</a>'))
            ],
            [
                cell(
                    content=_game_box_sub('<a href="{0}{1}3002.html">Detroit T'
                                          'igers 11, Chicago 4</a>'))
            ],
            [
                cell(
                    content=_game_box_sub('<a href="{0}{1}2993.html">Houston A'
                                          'stros 7, Seattle Mariners 2</a>'))
            ],
            [
                cell(
                    content=_game_box_sub('<a href="{0}{1}2991.html">Kansas Ci'
                                          'ty Royals 8, Cleveland Indians 2</a'
                                          '>'))
            ],
            [
                cell(
                    content=_game_box_sub('<a href="{0}{1}14721.html">Miami Ma'
                                          'rlins 6, Chicago 2</a>'))
            ],
            [
                cell(
                    content=_game_box_sub('<a href="{0}{1}3001.html">New York '
                                          '1, San Francisco Giants 0</a>'))
            ],
            [
                cell(
                    content=_game_box_sub('<a href="{0}{1}3000.html">New York '
                                          '5, Los Angeles 3</a>'))
            ],
            [
                cell(
                    content=_game_box_sub('<a href="{0}{1}2992.html">Philadelp'
                                          'hia Phillies 3, Washington National'
                                          's 1</a>'))
            ],
            [
                cell(
                    content=_game_box_sub('<a href="{0}{1}2999.html">San Diego'
                                          ' Padres 8, Colorado Rockies 2</a>'))
            ],
            [
                cell(
                    content=_game_box_sub('<a href="{0}{1}2990.html">St. Louis'
                                          ' Cardinals 5, Pittsburgh Pirates 4<'
                                          '/a>'))
            ],
            [
                cell(
                    content=_game_box_sub('<a href="{0}{1}2997.html">Tampa Bay'
                                          ' Rays 12, Boston Red Sox 9</a>'))
            ],
            [
                cell(
                    content=_game_box_sub('<a href="{0}{1}2994.html">Texas Ran'
                                          'gers 5, Oakland Athletics 3</a>'))
            ],
            [
                cell(
                    content=_game_box_sub('<a href="{0}{1}2995.html">Toronto B'
                                          'lue Jays 8, Minnesota Twins 2</a>'))
            ],
        ]
        expected = table(
            head=[cell(content='Sunday, October 9th, 2022')], body=body)
...
if __name__ in ['__main__', 'tasks.scoreboard.scoreboard_test']:
    _main = __name__ == '__main__'
    _pkg = 'tasks.scoreboard'
    _pth = 'tasks/scoreboard'
    main(ScoreboardTest, Scoreboard, _pkg, _pth, {}, _main, date=_now, e=_env)
